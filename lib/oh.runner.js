...
function logState(stack, done = false) {
    const state = chalk.dim(`${done ? ' done' : ''}`);
    const notification = `OH_${stack.reverse().join('_')}${state}`;
...
module.exports = class runner {
    ...
    run(task, stack = []) {
        ...
        return Promise.resolve(ohTask.runTask([task, ...stack])).then(() => {
            logState([task, ...stack], true);
        });
    }
};
